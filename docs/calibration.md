...
`sitecal` es una herramienta moderna (PWA y CLI) diseñada para realizar calibraciones de sitio de forma 100% offline. Permite transformar coordenadas globales geodéticas (Latitud, Longitud) en el datum WGS84 a un sistema de coordenadas plano local mediante diversos métodos de proyección y un motor de ajuste de similitud 2D.
...
2. **Proyección**: Las coordenadas globales se proyectan a un plano intermedio utilizando el método seleccionado (Default o LTM).
...
### 2. LTM (Local Transverse Mercator)
